...
class Web6Model extends Model {
    ...
    private function getCategory($spiderModel, $dom, $xPath) {
        $cat1 = $spiderModel->findNodeVal($dom, $xPath, "//div[contains(@class,'adress')]/h2/a[1]");
        $cat2 = $spiderModel->findNodeVal($dom, $xPath, "//div[contains(@class,'adress')]/h2/a[2]");
        return ['cat1' => $cat1, 'cat2' => trim($cat2)];
    }

    private function getTitle($spiderModel, $dom, $xPath) {
        $title = strip_tags($spiderModel->findNodeVal($dom, $xPath, "//div[contains(@class,'single')]/h1"));
        return trim($title);
    }

    private function getContentAndCover($spiderModel, $dom, $xPath, $entryUrl, $get_cover = 1) {
        $content = $spiderModel->findNodeVal($dom, $xPath, "//div[contains(@class,'singleCon')]/div");
    ...
    private function getPage($spiderModel, $dom, $xPath, $url) {
        $pageHtml = $spiderModel->findNodeVal($dom, $xPath, "//div[contains(@class,'singleCon')]/p");
