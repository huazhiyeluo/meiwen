...
class Web4Model extends Model {
    ...
    private function getCategory($spiderModel, $dom, $xPath) {
        $cat1 = $spiderModel->findNodeVal($dom, $xPath, "//div[contains(@class,'position')]/span/a[2]");
        $cat2 = $spiderModel->findNodeVal($dom, $xPath, "//div[contains(@class,'position')]/span/a[3]");
        return ['cat1' => $cat1, 'cat2' => trim($cat2)];
    }

    private function getTitle($spiderModel, $dom, $xPath) {
        $title = strip_tags($spiderModel->findNodeVal($dom, $xPath, "//div[contains(@class,'content-left')]/h1"));
        return trim($title);
    }

    private function getContentAndCover($spiderModel, $dom, $xPath, $entryUrl, $get_cover = 1) {
        $content = $spiderModel->findNodeVal($dom, $xPath, "//div[contains(@class,'content-left')]/div[contains(@class,'article')]");
