...
class Web5Model extends Model {
    ...
    private function getCategory($spiderModel, $dom, $xPath) {
        $cat1 = $spiderModel->findNodeVal($dom, $xPath, "//div[contains(@class,'place')]/a[2]");
        $cat2 = $spiderModel->findNodeVal($dom, $xPath, "//div[contains(@class,'place')]/a[3]");
        return ['cat1' => $cat1, 'cat2' => trim($cat2)];
    }

    private function getTitle($spiderModel, $dom, $xPath) {
        $title = strip_tags($spiderModel->findNodeVal($dom, $xPath, "//div[contains(@class,'title')]/h1"));
        return trim($title);
    }

    private function getContentAndCover($spiderModel, $dom, $xPath, $entryUrl, $get_cover = 1) {
        $content               = $spiderModel->findNodeVal($dom, $xPath, "//div[contains(@class,'content')]/div[1]");
        ...
        $content               = str_replace('<span >版权作品，未经《99作文网》书面授权，严禁转载，违者将被追究法律责任。</span>', '', $content);
        $content               = str_replace('<span >版权作品，未经《99作文网》书面授权，严禁转载，违者将被追究法律责任。</span>', '', $content);
        $insertData['content'] = str_replace('<span style="color:#ff0000;">版权作品，未经《99作文网》书面授权，严禁转载，违者将被追究法律责任。</span>', '', $content);
